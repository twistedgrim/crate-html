...
Static-site fixtures used by the (forthcoming) smoke-test harness. Each subdirectory is a self-contained site that should round-trip cleanly through `crate push` → daemon storage → HTTP GET → expected response.
